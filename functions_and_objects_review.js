...
// Output: 3
// ```
function avgNum3 (num1, num2, num3) {
...
function remEven(param) {
  for (let i = 0; i < param.length; i++) {
    if (param[i] % 2 !== 0) {
      evenArr.push(param[i]);
    }
  } return evenArr
}
...
// Output: false
// ```
let posArray = [-4,3,-2,4];
function posCheck (param) {
  return param > 0
}
console.log(posArray.every(posCheck))
...
// ```
let caps = ["HELLO", "THERE", "THESE", "ARE", "SOME", "STRINGS"];
function lower (param) {
  return param.toLowerCase();
}

console.log(caps.map(lower))
// BONUS:
...
let stringArr = ["It’s", "not", "a", "bug.", "It’s", "an", "undocumented", "feature!"]

let oneString = stringArr.reduce((acc, el) => {
  return acc + " " + el
})
...
function fahrenToCels (param) {
  let result = (param - 32) * 5 / 9
  return result
}

console.log(fahrenToCels(212))
